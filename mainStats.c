...
/*---------- begin */
  ...
  if (DEBUG_MODE) fd =fopen(strn,"w");
/*---------- define ntot and n (n=actual size of Loc_dat) */
for (ntot = 10000; ntot < 1000000; ntot=ntot*10) {
    // just a test
    //ntot = 10000;
    n = ntot/nprocs;
  ...
  /*---------- generate some data */
    ...
    /*---------- save data into an array and gather it into into root for later use */
    memcpy(sav_dat,Loc_dat,n*sizeof(int));
    MPI_Gather(sav_dat,n,MPI_INT,sav_dat,n,MPI_INT,0,MPI_COMM_WORLD);
  /*---------- initial sort local  data here Glob_dat used as temp. buffer */
  ...
  /*---------- call hypercube sort */ 
    ...
    if (err) {
      printf(" ERR : processors are not a power of 2\n");
      MPI_Finalize(); 
      return(1);
    }
  ...
  /*---------- now ready for gatgerv to gather all data */    
    ...
    if (myid == 0) {
      ...
      printf("Number of processors = %d \t ntot = %d\t\ttime = %e sec.  \n", nprocs, ntot, etim);
    }
    MPI_Barrier(MPI_COMM_WORLD);
  }
  if (myid == 0) printf("\n");
